...
class FashionIQ(torch.utils.data.Dataset):
    def __init__(self, path, gallery_all=False, name = 'dress',split = 'train',transform=None):
        ...
        self.path = path
        self.image_dir = self.path + 'image_resize'
        ...
        with open(os.path.join(self.caption_dir, "ref_caption.{}.{}.json".format(self.name, self.split)), 'r') as f:
            self.ref_captions = json.load(f)
        with open(os.path.join(self.split_dir, "split1.{}.{}.json".format(self.name, self.split)), 'r') as f:
    ...
    def get_all_texts(self):
        texts = []
        with open(os.path.join(self.caption_dir, "ref_caption.{}.{}.json".format(self.name, 'train')), 'r') as f:
