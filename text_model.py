...
class SimpleVocab(object):
    ...
    def add_text_to_vocab(self,text):   # 每个单词建立word2id 以及 wordcount 的字�?
    ...
    def threshold_rare_words(self, wordcount_threshold=3):  # 去掉出现的单词数量小�?的词
...
class TextLSTMModel(torch.nn.Module):
    ...
    def forward_encoded_texts(self, texts):
        ...
        # lstm
        lstm_output, last_hidden = self.forward_lstm_(etexts)      #lstm_output shape(length,batch,hidden_num*directions)

        
        return lstm_output.permute(1,0,2).contiguous(), last_hidden

    def forward_feature(self, texts,lengths):
        # to tensor
        #lengths = [len(t) for t in texts]
        #itexts = torch.zeros((np.max(lengths), len(texts))).long()
        #for i in range(len(texts)):
